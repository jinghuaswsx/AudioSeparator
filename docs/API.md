...
- **键**：`MD5(content) + effective_preset/model_filename + output_format + single_stem + response_mode`。
...
```json
{
  ...
  "goals": {
    "vocal_quality": "默认人声质量优先，映射 vocal_balanced",
    "background_preserve": "背景音乐/环境音保留优先，映射 instrumental_full",
    "background_clean": "背景更干净、尽量少人声残留，映射 instrumental_clean",
    "background_balanced": "背景保留与残留控制折中，映射 instrumental_balanced"
  },
  "presets": {
...
|---|---|---|---|---|
| `file` | 文件 | 是 | — | 音频或视频 |
| `ensemble_preset` | string | 否 | `vocal_balanced`（环境变量 `AS_DEFAULT_PRESET` 决定） | 见 `/presets`；显式 preset 优先级最高 |
| `model_filename` | string | 否 | — | 直接指定单个模型文件（高级，与 `ensemble_preset` 互斥；优先级低于 `ensemble_preset`、高于 `separation_goal`） |
| `separation_goal` | string | 否 | `vocal_quality` | 语义目标。仅在未传 `ensemble_preset` / `model_filename` 时生效；`background_preserve` 用于保留 BGM/环境音 |
